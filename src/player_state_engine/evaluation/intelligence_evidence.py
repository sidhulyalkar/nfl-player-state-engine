...
def _normalize_evidence_tier(value: EvidenceTier | int) -> EvidenceTier:
    try:
        return value if isinstance(value, EvidenceTier) else EvidenceTier(int(value))
    except (TypeError, ValueError) as exc:
        raise ValueError(f"Invalid intelligence evidence tier: {value!r}") from exc
...
def family_feature_columns(
    frame: pd.DataFrame,
    family: str,
    *,
    allowed_features: Iterable[str] | None = None,
) -> list[str]:
    allowed = set(frame.columns if allowed_features is None else allowed_features)
    columns = [
        column
        for column in frame.columns
        if column in allowed
        and _family_matches(column, family)
        and not _is_non_model_column(column)
    ]
    canonical_prefix = {
    ...
    if canonical_prefix is not None:
        columns.extend(
            column
            for column in frame.columns
            if column.startswith(canonical_prefix) and not _is_non_model_column(column)
        )
...
def build_incremental_feature_sets(
    ...
    official = family_feature_columns(
        frame, "official_availability", allowed_features=base_features
    )
    objective = family_feature_columns(
        frame, "objective_opportunity", allowed_features=base_features
    )
    news = family_feature_columns(frame, "structured_news", allowed_features=base_features)
    public = family_feature_columns(
        frame, "public_player_context", allowed_features=base_features
    )

    variants: dict[str, list[str]] = {
        "numerical_baseline": base,
        "official_availability": list(dict.fromkeys([*base, *official])),
    }
    variants["objective_reference"] = list(
        dict.fromkeys([*variants["official_availability"], *objective])
    )
    variants["structured_news"] = list(
        dict.fromkeys([*variants["objective_reference"], *news])
    )
...
def _shift_family_forward_in_time(frame: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    """Leakage sensitivity control: move next same-season player evidence backward."""
    ...
    *,
    ...
    for family, columns in family_columns.items():
        if variant == f"{family}_shuffled_control":
            return _shuffle_family(frame, columns, seed=seed)
        if variant == f"{family}_shifted_time_leakage_control":
            return _shift_family_forward_in_time(frame, columns)
    return frame
...
def run_incremental_intelligence_benchmark(
    ...
    *,
    ...
    """Run the frozen staircase and one-family-at-a-time controls."""

    safe_features = list(dict.fromkeys(base_features))
    variants = build_incremental_feature_sets(safe_features, frame)
    family_columns = {
        family: family_feature_columns(frame, family, allowed_features=safe_features)
        for family in _FAMILY_PREFIXES
    }
    runs: dict[str, AblationRun] = {}
    for name, features in variants.items():
        working = _variant_frame(
            frame,
            name,
...
def _engine_predictions(run: AblationRun, target: str) -> pd.DataFrame:
    frame = run.result.predictions.loc[
        run.result.predictions["method"].eq("quantile_engine")
    ].copy()
    required = {
        "season",
        "week",
...
def _paired_predictions(
    reference: AblationRun, candidate: AblationRun, target: str
) -> pd.DataFrame:
    ...
    if bool((~np.isclose(actual_reference, actual_candidate, equal_nan=False)).any()):
        raise ValueError("Paired ablation artifacts disagree on realized outcomes")
    if bool(
        (
            paired["position_reference"].astype(str)
            != paired["position_candidate"].astype(str)
        ).any()
    ):
...
def _paired_bootstrap(
    ...
    *,
    ...
    block_stats = data.groupby(["season", "week"], sort=False)[effect_column].agg(
        ["sum", "count"]
    )
...
def _strict_binary_series(series: pd.Series, *, label: str) -> pd.Series:
    """Parse explicit coverage/prevalence flags without converting garbage to false."""

    result = pd.Series(np.nan, index=series.index, dtype=float)
    if pd.api.types.is_bool_dtype(series.dtype):
        result.loc[series.notna()] = series.loc[series.notna()].astype(float)
        return result

    numeric = pd.to_numeric(series, errors="coerce")
    numeric_present = numeric.notna()
    invalid_numeric = numeric_present & ~numeric.isin([0.0, 1.0])
    if bool(invalid_numeric.any()):
        examples = sorted({str(value) for value in series.loc[invalid_numeric].head(5)})
        raise ValueError(f"{label} contains non-binary numeric values: {examples}")
    result.loc[numeric_present] = numeric.loc[numeric_present].astype(float)

    unresolved = ~numeric_present & series.notna()
    if bool(unresolved.any()):
        text = series.loc[unresolved].astype("string").str.strip().str.lower()
        mapping = {
            "true": 1.0,
            "yes": 1.0,
            "y": 1.0,
        ...
        invalid = mapped.isna() & text.ne("")
        if bool(invalid.any()):
            examples = sorted(set(text.loc[invalid].head(5).tolist()))
            raise ValueError(f"{label} contains unrecognized binary values: {examples}")
        result.loc[mapped.index] = mapped.to_numpy(dtype=float)
    return result


def _coalesced_binary_rate(
    frame: pd.DataFrame,
    columns: tuple[str, ...],
    *,
    label: str,
    ...
    for column in present:
        parsed = _strict_binary_series(frame[column], label=f"{label}:{column}")
        conflict = combined.notna() & parsed.notna() & ~np.isclose(combined, parsed)
        if bool(conflict.any()):
            raise ValueError(
                f"Conflicting {label} aliases on {int(conflict.sum())} evaluated row(s): "
                + ", ".join(present)
            )
        combined = combined.combine_first(parsed)
    measurement_rate = float(combined.notna().mean()) if len(combined) else 0.0
    positive_rate = float(combined.fillna(0.0).mean()) if len(combined) else 0.0
    return positive_rate, measurement_rate


def _conflict_rate(frame: pd.DataFrame, family: str) -> float | None:
    columns = [column for column in _CONFLICT_COLUMNS[family] if column in frame]
    if not columns:
        return None
    values = pd.concat(
        [pd.to_numeric(frame[column], errors="coerce") for column in columns], axis=1
    ).fillna(0.0)
    return float((values.max(axis=1) > 0.0).mean())
...
def evaluate_incremental_intelligence_evidence(
    ...
    target: str,
    *,
    evidence_tier: EvidenceTier | int = EvidenceTier.SYNTHETIC_ONLY,
    ...
    resolved_tier = _normalize_evidence_tier(evidence_tier)
    ...
    for candidate_index, (family, reference_name, candidate_name) in enumerate(
        ...
        paired = _paired_predictions(runs[reference_name], runs[candidate_name], target)
        primary = _paired_bootstrap(
            paired,
            bootstrap_samples=bootstrap_samples,
            seed=seed + 100 * candidate_index,
        )
        identity_pair = _paired_predictions(
            runs[f"{family}_shuffled_control"], runs[candidate_name], target
        )
        ...
        leakage_pair = _paired_predictions(
            runs[candidate_name], runs[f"{family}_shifted_time_leakage_control"], target
        )
        ...
        source_coverage, source_measurement_rate = _coalesced_binary_rate(
            paired_rows,
            _SOURCE_COVERAGE_COLUMNS[family],
            label=f"{family} source coverage",
        )
        claim_prevalence, claim_measurement_rate = _coalesced_binary_rate(
            paired_rows,
            _PREVALENCE_COLUMNS[family],
            label=f"{family} claim prevalence",
        )
        ...
        candidate_rows.append(
            {
                ...
                "evidence_tier": int(resolved_tier),
                "evidence_tier_name": resolved_tier.name.lower(),
                ...
                "source_coverage_measurement_rate": source_measurement_rate,
                "claim_prevalence": claim_prevalence,
                "claim_prevalence_measurement_rate": claim_measurement_rate,
    ...
    for row in candidate_rows:
        ...
        if int(row["paired_rows"]) < minimum_paired_rows:
            model_blockers.append("paired_rows_below_minimum")
        if int(row["paired_seasons"]) < minimum_seasons:
            model_blockers.append("multi_season_evidence_missing")
        if int(row["paired_blocks"]) < minimum_blocks:
            model_blockers.append("paired_blocks_below_minimum")
        if not np.isfinite(float(row["effect"])) or float(row["effect"]) <= minimum_effect:
            model_blockers.append("incremental_effect_not_positive")
        if not np.isfinite(float(row["ci_low"])) or float(row["ci_low"]) <= 0.0:
            model_blockers.append("incremental_effect_ci_not_positive")
        if q_value is None or not np.isfinite(q_value) or q_value > maximum_fdr_q:
            model_blockers.append("fdr_q_above_threshold_or_missing")
        for label in ("season", "position", "week"):
            value = float(row[f"{label}_consistency"])
            if not np.isfinite(value):
                model_blockers.append(f"missing_{label}_consistency")
            elif value < minimum_consistency:
                model_blockers.append(f"inconsistent_{label}_effect")
        if not bool(row["identity_control_passed"]):
            model_blockers.append("identity_negative_control_failed")
        if float(row["coverage_gap_regression"]) > maximum_overall_coverage_gap_regression:
            model_blockers.append("overall_calibration_regression")
        position_regression = row["max_supported_position_coverage_gap_regression"]
        if position_regression is None:
            model_blockers.append("position_calibration_not_measured")
        elif float(position_regression) > maximum_position_coverage_gap_regression:
            model_blockers.append("position_calibration_regression")

        row["model_gate_blockers"] = list(dict.fromkeys(model_blockers))
        row["model_gate_passed"] = not model_blockers

        activation_blockers = list(model_blockers)
        source_coverage = row["source_coverage"]
        measurement_rate = float(row["source_coverage_measurement_rate"])
        if source_coverage is None:
            activation_blockers.append("source_coverage_not_measured")
        else:
            if measurement_rate < 1.0:
                activation_blockers.append("source_coverage_incomplete")
            if float(source_coverage) < minimum_source_coverage:
                activation_blockers.append("source_coverage_below_threshold")
        if resolved_tier < EvidenceTier.MULTI_SEASON_ISOLATED:
            activation_blockers.append("evidence_tier_below_activation_review")
        row["activation_review_blockers"] = list(dict.fromkeys(activation_blockers))
        row["eligible_for_activation_review"] = not activation_blockers
