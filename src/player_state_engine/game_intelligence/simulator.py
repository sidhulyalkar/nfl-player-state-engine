...
def _bucket_distance(value: float) -> int:
    return 0 if value <= 2 else 1 if value <= 6 else 2 if value <= 10 else 3


def _field_zone(value: float) -> int:
    return 0 if value <= 20 else 1 if value <= 40 else 2 if value <= 60 else 3 if value <= 80 else 4


def _quarter(clock: float) -> int:
    return 1 if clock > 2700 else 2 if clock > 1800 else 3 if clock > 900 else 4


def _numeric_weights(frame: pd.DataFrame, column: str) -> pd.Series:
...
def _select_player(
    ...
    *,
    ...
    weights = _numeric_weights(pool, str(weight_column))
    if float(weights.sum()) <= 0:
        weights = _numeric_weights(pool, "usage_evidence_weight")
    if float(weights.sum()) <= 0:
...
def _pass_probability(
    ...
    *,
    offense_spread: float,
    game_total: float,
) -> float:
    probability = float(profile.get("pregame_pass_rate", 0.58))
    row = {
        **profile,
        **state,
    ...
    if model is not None and model.fitted:
        probability = float(model.predict_pass_probability(pd.DataFrame([row]))[0])
    ...
    if state["down"] == 1 and state["ydstogo"] <= 2:
        probability -= 0.07
    return float(np.clip(probability, 0.20, 0.90))
...
def _fourth_down_action(
    ...
    if rng.random() < go_probability:
        return "GO"
    return "FIELD_GOAL" if yardline_100 <= 35 else "PUNT"
...
def _reset_possession(offense: str, home: str, away: str) -> tuple[str, str, float, int, float]:
    new_offense = away if offense == home else home
    new_defense = home if new_offense == away else away
...
def _summarize(frame: pd.DataFrame, group: list[str], value: str) -> pd.DataFrame:
    if frame.empty:
...
def simulate_matchup(
    ...
    *,
    ...
    for simulation in range(config.simulations):
        ...
        yardline, down, distance, clock = 75.0, 1, 10.0, 3600.0
        play_index = 0
        ...
        while clock > 0 and play_index < config.max_plays:
            ...
            if down == 4:
                action = _fourth_down_action(
                    yardline, distance, rng, config.fourth_down_aggression_scale
                )
                if action == "FIELD_GOAL":
                    scores[offense] += 3.0 if rng.random() < _field_goal_success(yardline) else 0.0
                    clock -= min(6.0, clock)
                    offense, defense, yardline, down, distance = _reset_possession(
            ...
            family = "DROPBACK" if rng.random() < pass_probability else "RUSH"
            plays[offense] += 1
            dropbacks[offense] += int(family == "DROPBACK")
            ...
            if not np.isfinite(runoff) or runoff <= 0:
                runoff = 30.0 if family == "RUSH" else 24.0
            clock = max(
                0.0,
                clock
                - float(
            ...
            if before_clock > 1800 >= clock:
                ...
                yardline, down, distance = 75.0, 1, 10.0
