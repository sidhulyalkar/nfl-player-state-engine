...
def simulate_matchup_transition_probe(
    ...
    *,
    ...
    if game_usage.empty:
        raise ValueError("Transition probe requires point-in-time usage for a matchup team")
        ...
        while clock > 0 and play_index < config.max_plays:
            ...
            if down == 4:
                ...
                if action == "FIELD_GOAL":
                    ...
                    made = bool(rng_special.random() < probability)
    ...
    diagnostics = {
        "game_id": game_id,
        "model_source": "possession_transition_probe_v014",
        "promoted": False,
        "simulations": config.simulations,
        ...
        "limitations": [
            ...
            "touchdowns still use seven points instead of a separate PAT/two-point model",
            "fourth-down action selection remains a transparent heuristic",
        ],
    }
