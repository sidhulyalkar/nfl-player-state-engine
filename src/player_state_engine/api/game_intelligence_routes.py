...
def install_game_intelligence_routes(
    ...
    *,
    ...
    benchmark = Path(
        benchmark_root
        or os.getenv("PSE_GAME_INTELLIGENCE_BENCHMARK_ROOT", "artifacts/game_intelligence/v014")
    )
    ...
    def game_intelligence_status() -> dict[str, object]:
        ...
        return {
            "model_family": "game_intelligence_v014_research",
    ...
    def game_intelligence_benchmark() -> dict[str, object]:
        ...
        if summary is None:
            raise HTTPException(
                status_code=503,
                detail=f"No v0.14 game-intelligence benchmark summary available under {benchmark}",
            )
