    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "authority_note": (
                ...
                "exception is explicitly isolated to a declared non-model lane such as market-only "
                "K/DST or an optional NFL Hub source outage."
            ),
        }
...
def _league_release_status(
    ...
    *,
    ...
    if not unexpected_blockers and isolated_market_only:
    ...
    reasons = tuple(f"LEAGUE_{name}:{flag}" for flag in report.blocking_flags)
    if not reasons:
...
def assess_sept1_release_readiness(
    ...
    *,
    ...
    when every unresolved league exception is isolated to explicitly declared market-only
    positions and/or the NFL Hub has only optional-source degradation. Neither state can be
    reached without an immutable, production-approved projection bundle and fresh core inputs.
    """
