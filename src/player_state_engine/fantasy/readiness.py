...
def assess_league_readiness(projections: pd.DataFrame, config: LeagueConfig) -> LeagueReadinessReport:
    ...
    market_column = "market_adp" if "market_adp" in projections else "market_cost" if "market_cost" in projections else None
    if market_column is None or rows == 0:
        market_coverage = 0.0
        flags.append("MISSING_MARKET_DATA")
    else:
        market_coverage = float(pd.to_numeric(projections[market_column], errors="coerce").notna().mean())
