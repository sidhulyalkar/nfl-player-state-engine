...
def simulate_draft_room(
    ...
    *,
    ...
    adp_sd_values = (
        adp_sd.fillna(pd.Series(default_sd, index=board.index)).clip(lower=1.0).to_numpy(float)
    )
    ...
    already = {
        str(key).upper(): int(value) for key, value in (drafted_position_counts or {}).items()
    }
    initial_remaining = {
    ...
    for simulation_index in range(cfg.simulations):
        ...
        for _ in range(min(picks_between, n_players)):
            ...
            if total_remaining > 1e-12:
                pressure = np.asarray(
                    [
                        remaining.get(positions[index], 0.0) / total_remaining
    ...
    survival_se = np.sqrt(
        np.clip(survival * (1.0 - survival) / float(cfg.simulations), 0.0, None)
    )
