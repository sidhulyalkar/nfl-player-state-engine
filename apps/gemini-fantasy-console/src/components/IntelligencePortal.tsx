...
export function IntelligencePortal() {
  ...
  return <div className="portal-shell">
    <header className="portal-header">
      ...
      <div className="portal-actions"><label><span>League</span><select value={leagueId} onChange={(event) => setLeagueId(event.target.value)}><option value="demo-league">Visual demo</option>{leagues.map((league) => <option key={`${league.platform}-${league.league_id}`} value={league.league_id}>{league.name}</option>)}</select></label>{loading && <RefreshCw size={17} className="spin"/>}</div>
    </header>
