...
The draft product needs one operator-facing answer that is harder to misread than a collection of individually green dashboards. This gate combines projection authority, byte integrity, source freshness, current NFL state, market identity health and exact fantasy league contracts into one release verdict.
...
- K/DST are the only required positions that remain missing or scoring-inexact, and they are declared `market_only_positions`; and/or
...
- any other league-readiness blocker that is not solely a declared market-only K/DST exception.
...
8. run this release gate across every real league contract;
9. rehearse the live Draft Room using the exact frozen artifacts;
10. bump/freeze v0.17.0 and rerun `--strict-ready` immediately before the release candidate is used.

A green CI run is necessary but not sufficient. The release verdict is tied to the actual projection bytes, actual NFL snapshot and actual league contracts that will be used at draft time.
