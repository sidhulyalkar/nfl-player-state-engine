...
def _annotate_manifest() -> None:
    ...
    payload["historical_injury_adapter"] = {
        ...
        "reason": (
            "The frozen nflverse injury archive uses long-form practice strings. The canonical "
            "historical intelligence corpus now maps those exact source values directly."
        ),
    }
    payload["interpretation_boundary"] = (
        "Primary fantasy-relevant regular-season experiment on the exact frozen v2 numerical "
        "and injury source bytes. nflverse long-form practice statuses are interpreted directly "
        "by the canonical historical corpus adapter. Postseason rows may inform later lagged player "
        "histories when chronologically prior, but they are excluded from model fitting and "
        "evaluation. This remains research evidence only and grants no automatic production authority."
    )
