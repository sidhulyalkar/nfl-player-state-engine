...
def _report(payload: dict[str, object]) -> str:
    ...
    pace = payload["aggregate_pace_metrics"]
    ...
    comparison = ("learned_state_legacy", "learned_state_drive")
    lines = [
        "# v0.13 Drive Volume & State Replay",
        "",
        "Research evidence only. No result automatically changes production projections.",
        "",
        "## Primary champion comparison",
        "",
        "| Metric | learned_state_legacy | learned_state_drive |",
        "|---|---:|---:|",
    ]
    metrics = sorted(
        {
    ...
    lines.extend(
        [
            "",
            "## Isolated pace negative control",
            "",
            "| Metric | Value |",
            "|---|---:|",
        ]
    )
    for metric, value in sorted(pace.items()):
        lines.append(f"| {metric} | {float(value):.6f} |")

    lines.extend(
        [
            "",
            "## Drive-volume research gate",
            "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the v0.13 eight-cell drive-volume expanding frozen replay."
    )
    ...
    parser.add_argument("--output-dir", default="artifacts/game_intelligence/v013")
...
def main() -> None:
    ...
    benchmark = run_v013_drive_volume_benchmark(
        pbp,
        schedules,
    ...
    benchmark.weekly_metrics.to_parquet(
        output / "weekly_drive_factorial_metrics.parquet", index=False
    )
    benchmark.weekly_pace_metrics.to_parquet(
        output / "weekly_pace_negative_control.parquet", index=False
    )
    payload: dict[str, object] = {
        "aggregate_metrics": benchmark.aggregate_metrics,
        "aggregate_pace_metrics": benchmark.aggregate_pace_metrics,
