...
def _require_direct_config_identity(
    direct_manifest_path: Path,
    league_paths: tuple[Path, ...],
    leagues: dict[str, LeagueConfig],
) -> None:
    """Reject stale evidence that only happens to share a human-readable league slug."""

    manifest = _json(direct_manifest_path)
    raw_entries = manifest.get("leagues")
    if not isinstance(raw_entries, dict):
        raise ValueError("Direct-score evidence is missing league entries")
    paths_by_slug = {_slug(path): path for path in league_paths}
    if set(paths_by_slug) != set(leagues):
        raise ValueError("Current league paths do not match the reviewed release league set")

    for slug, league in leagues.items():
        entry = raw_entries.get(slug)
        if not isinstance(entry, dict):
            raise ValueError(f"Direct-score evidence is missing league {slug!r}")
        config_record = entry.get("league_config")
        if not isinstance(config_record, dict):
            raise ValueError(f"Direct-score evidence lacks config identity for {slug}")
        current_path = paths_by_slug[slug]
        recorded_sha = str(config_record.get("sha256") or "").strip()
        current_sha = sha256_file(current_path)
        if not recorded_sha or recorded_sha != current_sha:
            raise ValueError(
                f"Direct-score evidence config SHA does not match current {slug} YAML: "
                f"{recorded_sha or '<missing>'} != {current_sha}"
            )
        if int(entry.get("teams", -1) or -1) != int(league.teams):
            raise ValueError(f"Direct-score evidence team count does not match current {slug}")
        if str(entry.get("scoring") or "").strip().lower() != str(league.scoring).strip().lower():
            raise ValueError(f"Direct-score evidence scoring mode does not match current {slug}")
        if bool(entry.get("median_scoring")) != bool(league.median_scoring):
            raise ValueError(f"Direct-score evidence median flag does not match current {slug}")
...
def main() -> None:
    ...
    _require_direct_config_identity(direct_manifest_path, league_paths, leagues)
    ...
    if market_context.empty:
        ...
        market_rank_rows = int(market_context["market_rank"].notna().sum())
        market_adp_rows = int(market_context["market_adp"].notna().sum())
    if market_rank_rows < 250:
        raise RuntimeError(
            "Current NFL Hub exact-ID market ordering is too sparse for release materialization: "
            f"market_rank_rows={market_rank_rows}"
        )
    ...
    if len(current_features) < 250:
        raise RuntimeError(
            f"Current preseason skill-player universe is unexpectedly small: {len(current_features)}"
        )
        ...
        if contract_evidence.decision_quantile_policy == PPR_POLICY:
            ...
            if not source_calibrator.is_file():
                raise FileNotFoundError(
                    "Qualified PPR calibrator is missing from uncertainty evidence: "
                    f"{source_calibrator}"
                )
    ...
    product = combine_contract_product_frames(contract_frames)
    ...
    contract_metadata = {
        slug: {
            **item.as_dict(),
            ...
            "rows": int(len(contract_frames[slug])),
            "market_rank_coverage": float(contract_frames[slug]["market_rank"].notna().mean()),
            "market_adp_coverage": float(contract_frames[slug]["market_adp"].notna().mean()),
        }
    ...
    qualification = {
        ...
        "nfl_hub": {
            ...
            "market_rank_rows": market_rank_rows,
            "market_adp_rows": market_adp_rows,
            "market_identity": hub.get("market_identity"),
            "adp_timing_authority": (
                "available" if market_adp_rows else "unavailable_do_not_substitute_market_rank"
            ),
        },
        "release_boundary": (
            "This bundle is a challenger only. Market rank is preserved as external ordering and "
            "must not be relabeled as ADP. Run the multicontract release rehearsal against fresh "
            "NFL Hub and external-only K/DST evidence, then create a separate manually approved "
            "production manifest over the exact same bytes before champion promotion."
        ),
    }
